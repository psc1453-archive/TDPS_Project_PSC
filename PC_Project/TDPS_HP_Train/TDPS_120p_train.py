...
from TDPS_CNN_120p import TDPS_CNN_120p
...
train_dataset = TDPS_HP_DataSet(root='/run/media/psc/LinuxData/Carset/Classified/Cropped', resolution=resolution,
...
for times in range(100):
    ...
    if tc.cuda.is_available():
        model = TDPS_CNN_120p().cuda().half()
        tc.save(model.state_dict(), 'net120/initial.pth')
    else:
        model = TDPS_CNN_120p()
    ...
    plt.plot(times, achieve)
    plt.plot(times, loss_list)
    plt.savefig('./net120/net' + str(max_accuracy) + '.jpg')
    plt.show()
    tc.save(model.state_dict(), './net120/net' + str(max_accuracy) + '.pth')

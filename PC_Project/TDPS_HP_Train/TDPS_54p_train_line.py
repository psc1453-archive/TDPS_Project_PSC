...
from TDPS_CNN_54p_line import TDPS_CNN_54p
...
train_dataset = TDPS_HP_DataSet(root='/run/media/psc/LinuxData/lineset/', resolution=resolution,
                                train=True, transform=data_tf)
test_dataset = TDPS_HP_DataSet(root='/run/media/psc/LinuxData/lineset/', resolution=resolution,
                               train=True, transform=data_tf)
...
for times in range(100):
    ...
    if tc.cuda.is_available():
        ...
        # model.load_state_dict(tc.load('net54/newnet0.0045928265899419785.pth', map_location='cuda'))
    else:
        model = TDPS_CNN_54p()
    ...
    plt.plot(times, achieve)
    plt.plot(times, loss_list)
    plt.savefig('./net_line/' + str(max_accuracy) + '.jpg')
    plt.show()
    tc.save(model.state_dict(), './net_line/' + str(max_accuracy) + '.pth')

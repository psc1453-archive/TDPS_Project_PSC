...
void Camera_Init_Device(I2C_HandleTypeDef *hi2c, framesize_t framesize, pixformat_t pixformat)
{
	hcamera.hi2c = hi2c;
	...
	else
	{
		...
		if (hcamera.manuf_id == 0x7fa2 && ((hcamera.device_id - 0x2641) <= 2))
		{
			...
			ov2640_init(framesize, pixformat);
		}
		else
		{
			...
			if (hcamera.manuf_id == 0x7fa2 && ((hcamera.device_id - 0x7721) <= 2))
			{
				...
				// 但是使用TIM1输出XCLK时钟后与LCD的背光PWM冲突，故该函数自动设置LCD使用软件PWM控制
				// Camera_XCLK_Set(XCLK_TIM);
				ov7725_init(framesize, pixformat);
			}
			else
			{
				...
				{
					// 自动对焦 AF 未实现
					ov5640_init(framesize, pixformat);
				}
				else
				{
					hcamera.addr = 0;
